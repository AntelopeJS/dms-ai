...
if (!Number.isInteger(listenPort)) throw new Error("PORT must be an integer");

function selectTarget(requestUrl = "/") {
  const parsedUrl = new URL(requestUrl, "http://gateway.internal");
  const path = `${parsedUrl.pathname}${parsedUrl.search}`;
  const isBackend = path === backendPrefix || path.startsWith(`${backendPrefix}/`);
  return { target: isBackend ? backendUrl : frontendUrl, path: isBackend ? path.slice(backendPrefix.length) || "/" : path };
}
...
const server = http.createServer((request, response) => {
  ...
  const upstream = http.request(new URL(path, target), { method: request.method, headers: upstreamHeaders(request, target) }, (upstreamResponse) => {
    response.writeHead(upstreamResponse.statusCode ?? 502, upstreamResponse.statusMessage, responseHeaders(upstreamResponse.headers, target));
...
server.on("upgrade", (request, socket, head) => {
  const { target, path } = selectTarget(request.url);
  const upstream = http.request(new URL(path, target), { method: request.method, headers: upstreamHeaders(request, target, true) });
  upstream.on("upgrade", (upstreamResponse, upstreamSocket, upstreamHead) => {
    const statusLine = `HTTP/1.1 ${upstreamResponse.statusCode ?? 101} ${upstreamResponse.statusMessage ?? "Switching Protocols"}`;
    const headers = [];
    for (let index = 0; index < upstreamResponse.rawHeaders.length; index += 2) headers.push(`${upstreamResponse.rawHeaders[index]}: ${upstreamResponse.rawHeaders[index + 1]}`);
